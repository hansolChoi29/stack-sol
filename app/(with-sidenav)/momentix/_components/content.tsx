...
import { MOMENTIXPOSTS } from "@/app/data/protect";
import { useMemo, useState } from "react";
import { Input } from "@/components/ui/input";

export function MomentixContent() {
  const router = useRouter();
  const [q, setQ] = useState("");

  const rows = useMemo(() => {
    const query = q.trim().toLowerCase();
    if (!query) return MOMENTIXPOSTS;

    return MOMENTIXPOSTS.filter((p) => {
      return (
        String(p.title ?? "")
          .toLowerCase()
          .includes(query) ||
  ...
  return (
    <section className="hero-shell min-h-[100dvh] bg-white text-[#241E7A]">
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div className="w-full sm:max-w-md">
            <Input
              ...
              className="bg-white text-black border-black/15 focus-visible:ring-[#241E7A]"
            />
          </div>
            ...
            <TableBody>
              {rows.length === 0 ? (
                ...
                rows.map((r) => (
                  <TableRow
                    key={r.id}
                    onClick={() => router.push(`/momentix/${r.id}`)}
                  ...
                  >
                    ...
                    <TableCell className="text-black">
                      <div className="flex items-center gap-2">
                        ...
                        {r.status === "트러블슈팅" && (
                          <Badge className="bg-[#241E7A] text-white hover:opacity-90">
                            트러블슈팅
                          </Badge>
                        )}
                        {r.status === "성능 개선" && (
